...
function Card({ title, block, delay }: { title: string; block: Block; delay: number }) {
  ...
  return (
    <div
      ...
      style={{ animation: `surface-enter 0.45s cubic-bezier(0.22, 1, 0.36, 1) ${delay}ms both` }}
    >
      <div
        aria-hidden
      ...
      <div className="relative mt-4 space-y-1.5 border-t border-line pt-4 text-sm leading-relaxed text-ink-soft">
        ...
        <p className="flex items-baseline justify-between gap-3">
          <span>{t('pl.cogs')}</span>
          <span className="font-medium text-ink tabular-nums">{money(block.cogs, numberLocale)}</span>
        </p>
        <p className="flex items-baseline justify-between gap-3">
...
export function PlPage() {
  ...
  return (
    <div className="font-pl">
      ...
      <Surface>
        {!data ? (
          <p className="py-8 text-center text-ink-muted italic">{t('common.loading')}</p>
        ) : (
