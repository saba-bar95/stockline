...
export function SettingsPanel() {
  const { t, locale, setLocale, theme, setTheme, fontSize, cycleFontSize } = usePrefs()
  const [open, setOpen] = useState(false)
  const [pos, setPos] = useState<PanelPos>({ bottom: 72, left: 16, width: 280, maxHeight: 520 })
        ...
        >
          ...
          <div className="space-y-4">
            ...
            <fieldset className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-1.5">
                <SegBtn active={theme === 'light'} onClick={(e) => onThemeClick('light', e)}>
                  {t('settings.themeLight')}
                </SegBtn>
                <SegBtn active={theme === 'dark'} onClick={(e) => onThemeClick('dark', e)}>
            ...
            <fieldset className="space-y-2">
              ...
              >
                ...
                >
                  {FONT_LABEL[fontSize]}
                  <span className="relative -top-1 ml-0.5 text-[0.65em]">+</span>
                </span>
                <span className="text-sm text-ink-muted tabular-nums">
                  {fontSize === 'sm' ? 'S' : fontSize === 'md' ? 'M' : 'L'}
                </span>
              </button>
              <p className="text-xs text-ink-muted">{t('settings.fontSizeHint')}</p>
            </fieldset>
            ...
            <fieldset className="space-y-2">
              ...
              <p className="text-xs text-ink-muted">{t('settings.exportHint')}</p>
              <div className="grid grid-cols-1 gap-1.5">
                <button
                  ...
                  onClick={async () => {
                    ...
                    setExportErr('')
                    try {
                      await downloadExport('/export/workbook.xlsx', 'mise-export.xlsx')
                    } catch (e) {
