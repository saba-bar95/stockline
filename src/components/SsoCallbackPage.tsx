...
import { OAUTH_ERROR_PARAM, takeOAuthIntent, type OAuthErrorCode } from '../lib/oauthFlow'
...
function redirectWithError(
  ...
  navigate(`/${locale}/${target}?${OAUTH_ERROR_PARAM}=${code}`, { replace: true })
}
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const accountAgeMs =
          user?.createdAt != null ? Date.now() - new Date(user.createdAt).getTime() : 0
