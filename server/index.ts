...
async function ingredientHasOps(orgId: string, id: string): Promise<boolean> {
    const purchase = await qGet(db
        .select({ id: purchases.id })
        .from(purchases)
        .where(and(eq(purchases.organizationId, orgId), eq(purchases.kind, "Ingredient"), eq(purchases.itemId, id))));
    if (purchase)
        return true;
    const writeOff = await qGet(db
        .select({ id: writeOffs.id })
        .from(writeOffs)
        .where(and(eq(writeOffs.organizationId, orgId), eq(writeOffs.kind, "Ingredient"), eq(writeOffs.itemId, id))));
    if (writeOff)
        return true;
    const recipe = await qGet(db
...
app.get("/ingredients", async (c) => {
    ...
    return c.json(await Promise.all(rows.map(async (r) => ({
        ...r,
...
app.get("/ingredients/:id/history", async (c) => {
    ...
    const ing = await qGet(db
        .select()
        .from(ingredients)
        .where(and(eq(ingredients.organizationId, orgId), eq(ingredients.id, id))));
    if (!ing)
        return c.json({ error: "არ მოიძებნა" }, 404);
    const pur = (await qAll(db
        .select()
        .from(purchases)
        .where(and(eq(purchases.organizationId, orgId), eq(purchases.kind, "Ingredient"), eq(purchases.itemId, id)))
        .orderBy(desc(purchases.date)))).map((p) => ({
        date: p.date,
        type: "შესყიდვა",
        qty: p.qty,
    ...
    const prodNames = Object.fromEntries((await qAll(db
        .select()
        .from(products)
        .where(eq(products.organizationId, orgId)))).map((p) => [p.id, p.name]));
    const used: Array<{
        date: string;
        type: string;
        qty: number;
    ...
    for (const run of await qAll(db
        .select()
        .from(productionRuns)
        .where(eq(productionRuns.organizationId, orgId)))) {
        const line = await qGet(db
            .select()
            .from(recipeLines)
            .where(and(eq(recipeLines.organizationId, orgId), eq(recipeLines.productId, run.productId), eq(recipeLines.ingredientId, id))));
        if (!line)
            continue;
    ...
    const wo = await Promise.all((await qAll(db
        .select()
        .from(writeOffs)
        .where(and(eq(writeOffs.organizationId, orgId), eq(writeOffs.kind, "Ingredient"), eq(writeOffs.itemId, id)))
        .orderBy(desc(writeOffs.date)))).map(async (w) => ({
        date: w.date,
        type: "ჩამოწერა",
        qty: -w.qty,
...
app.post("/ingredients", async (c) => {
    ...
    const body = z
        ...
        .parse(await c.req.json());
    const id = newId();
    await qRun(db.insert(ingredients)
        .values({
        id,
...
app.delete("/ingredients/:id", async (c) => {
    ...
    await qRun(db.delete(ingredients)
        .where(and(eq(ingredients.organizationId, orgId), eq(ingredients.id, id))));
...
app.post("/resale", async (c) => {
    ...
    const body = z
        ...
        .parse(await c.req.json());
    const id = newId();
    await qRun(db.insert(resaleProducts)
        .values({
        id,
        organizationId: orgId,
        name: body.name,
...
app.get("/recipes", async (c) => {
    ...
    const ings = Object.fromEntries((await qAll(db
        .select()
        .from(ingredients)
        .where(eq(ingredients.organizationId, orgId)))).map((i) => [i.id, i]));
    const prods = Object.fromEntries((await qAll(db
        .select()
        .from(products)
        .where(eq(products.organizationId, orgId)))).map((p) => [p.id, p]));
    return c.json(lines.map((l) => ({
        ...l,
...
app.post("/recipes", async (c) => {
    ...
    await qRun(db.insert(recipeLines)
        .values({
        organizationId: orgId,
...
app.delete("/recipes/:id", async (c) => {
    const orgId = getOrg(c);
    await qRun(db.delete(recipeLines)
        .where(and(eq(recipeLines.organizationId, orgId), eq(recipeLines.id, Number(c.req.param("id"))))));
    return c.json({ ok: true });
});
// —— Purchases ——
app.get("/purchases", async (c) => {
    const orgId = getOrg(c);
    return c.json(await qAll(db
        .select()
        .from(purchases)
...
app.post("/purchases", async (c) => {
    ...
    await qRun(db.insert(purchases)
        .values({
        organizationId: orgId,
        date: body.date,
        kind: body.kind,
...
app.get("/production", async (c) => {
    ...
    const names = Object.fromEntries((await qAll(db
        .select()
        .from(products)
        .where(eq(products.organizationId, orgId)))).map((p) => [p.id, p.name]));
    return c.json(await Promise.all(rows.map(async (r) => {
        const unitSnap = r.ingredientUnitCost > 0
            ? r.ingredientUnitCost
            : await recipeUnitCost(orgId, r.productId);
        const ingTotal = await runIngredientTotal(orgId, r);
        const ohTotal = await runOverheadTotal(orgId, r.date, r.productId, r.qty, r.id);
        return {
            ...r,
...
app.post("/production", async (c) => {
    ...
    await qRun(db.insert(productionRuns)
        .values({
        organizationId: orgId,
        date: body.date,
        productId: body.productId,
...
app.post("/sales", async (c) => {
    ...
    const body = z
        ...
        .parse(await c.req.json());
    const stock = body.source === "manufactured"
        ? await productStock(orgId, body.itemId) : await resaleStock(orgId, body.itemId);
    if (stock + 1e-9 < body.qty) {
        return c.json({ error: `არასაკმარისი ნაშთი (არის ${stock})` }, 400);
    }
    await qRun(db.insert(sales)
        .values({
        organizationId: orgId,
        date: body.date,
        source: body.source,
...
app.post("/write-offs", async (c) => {
    ...
    const body = z
        ...
        .parse(await c.req.json());
    await qRun(db.insert(writeOffs)
        .values({
        organizationId: orgId,
        date: body.date,
        kind: body.kind,
...
app.post("/employees", async (c) => {
    ...
    const body = z
        ...
        .parse(await c.req.json());
    const id = newId();
    await qRun(db.insert(employees)
        .values({
        id,
        organizationId: orgId,
        name: body.name,
...
app.post("/expenses", async (c) => {
    ...
    const body = z
        ...
        .parse(await c.req.json());
    let gel = body.gel ?? 0;
    if ((body.usd ?? 0) > 0 && (body.rate ?? 0) > 0)
        gel = body.usd! * body.rate!;
    await qRun(db.insert(expenses)
        .values({
        organizationId: orgId,
        date: body.date,
        type: body.type,
...
app.get("/pl", async (c) => {
    ...
    return c.json({
        day: await plSummary(orgId, dayStart, dayEnd),
        week: await plSummary(orgId, weekStart, weekEnd),
        month: await plSummary(orgId, monthStart, monthEnd),
    });
});
