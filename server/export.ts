...
const { employees, expenses, ingredients, payroll, productionRuns, products, purchases, recipeLines, resaleProducts, sales, writeOffs } = s;
...
async function orgTables(orgId: string) {
    ...
    return {
        ingredients: await qAll(db
            .select()
            .from(ingredients)
            .where(filter(ingredients))
            .orderBy(ingredients.name)),
        products: await qAll(db
            .select()
            .from(products)
            .where(filter(products))
            .orderBy(products.name)),
        resale: await qAll(db
            .select()
            .from(resaleProducts)
            .where(filter(resaleProducts))
            .orderBy(resaleProducts.name)),
        recipes: await qAll(db.select().from(recipeLines).where(filter(recipeLines))),
        purchases: await qAll(db
            .select()
            .from(purchases)
            .where(filter(purchases))
            .orderBy(desc(purchases.date))),
        production: await qAll(db
            .select()
            .from(productionRuns)
