...
function orgTables(orgId: string) {
  const filter = (table: { organizationId: unknown }) => eq(table.organizationId as never, orgId)
  return {
    ingredients: db.select().from(ingredients).where(filter(ingredients)).orderBy(ingredients.name).all(),
    products: db.select().from(products).where(filter(products)).orderBy(products.name).all(),
    resale: db.select().from(resaleProducts).where(filter(resaleProducts)).orderBy(resaleProducts.name).all(),
    recipes: db.select().from(recipeLines).where(filter(recipeLines)).all(),
    purchases: db.select().from(purchases).where(filter(purchases)).orderBy(desc(purchases.date)).all(),
    production: db.select().from(productionRuns).where(filter(productionRuns)).orderBy(desc(productionRuns.date)).all(),
    sales: db.select().from(sales).where(filter(sales)).orderBy(desc(sales.date)).all(),
    writeOffs: db.select().from(writeOffs).where(filter(writeOffs)).orderBy(desc(writeOffs.date)).all(),
    employees: db.select().from(employees).where(filter(employees)).orderBy(employees.name).all(),
    payroll: db.select().from(payroll).where(filter(payroll)).orderBy(desc(payroll.date)).all(),
    expenses: db.select().from(expenses).where(filter(expenses)).orderBy(desc(expenses.date)).all(),
  }
}
...
export async function handleExportCsv(c: Context, orgId: string, entity: string) {
