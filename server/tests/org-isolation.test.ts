...
const { ingredients, memberships, organizations } = s;
...
describe("org isolation", () => {
  ...
  it("returns 404 when deleting another org ingredient", async () => {
    ...
    expect(res.status).toBe(404);
    const stillThere = await qGet(
      db.select().from(ingredients).where(eq(ingredients.id, ING_B)),
    );
