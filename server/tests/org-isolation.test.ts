...
function api(path: string, init: RequestInit & { userId: string; orgId: string }) {
...
describe('org isolation', () => {
  ...
  it('lists only ingredients belonging to the caller org', async () => {
    const res = await api('/api/ingredients', { method: 'GET', userId: USER_A, orgId: ORG_A })
    expect(res.status).toBe(200)
  ...
  it('creates ingredients under the caller org only', async () => {
    ...
    const listA = await api('/api/ingredients', { method: 'GET', userId: USER_A, orgId: ORG_A })
    const idsA = ((await listA.json()) as Array<{ id: string }>).map((r) => r.id)
